...
// The DAO is an object or an interface that provides access to an underlying database/persistence storage.
public class ProductDAO {
    String[] productEntryArray = new String[]{ProductTable.COLUMN_PRODUCT_ID, ProductTable.COLUMN_PRODUCT_NAME, ProductTable
            .COLUMN_PRODUCT_CATEGORY, ProductTable.COLUMN_PRODUCT_EXPIRY, ProductTable.COLUMN_SHOPPING_CHECK, ProductTable
            .COLUMN_STOCKED, ProductTable.COLUMN_CONSUMED, ProductTable.COLUMN_EXPIRED};

    private SQLiteDatabase db;

    // Constructor - ProductDAO // @param db - SQLite database
    public ProductDAO(SQLiteDatabase db) {
        this.db = db;
    }

    // This will save the mentioned product item
    // @param productItem - specific product item @return inserts in DB
    ...
    // @param productItem - specific product item @return updates the DB
    public boolean update(Product productItem) {
        ...
        // the db.update() method will return INT for number of rows updated & so return db.update()>0 will check
    ...
    // @param productItem - specific item   @return deletes item from DB
    ...
    // @param id - product ID @return productItem
